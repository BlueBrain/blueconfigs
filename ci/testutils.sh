...
get_reference_file() {
    ...
    if [[ ! -f "${ref_file}" ]]; then
        ...
        if [[ ! -z $largest_version_less_than_desired_version_filename ]]; then
            ...
            local alternative_compiler_version_file=$(ls ${ref_results}/${nameroot}_v${REFERENCE_REPORTS_VERSION}${coreneuron_suffix}_${BUILD_COMPILER}_*.h5 2> /dev/null || find ${ref_results} -name "${nameroot}_v*[0-9]${coreneuron_suffix}_${BUILD_COMPILER}_*.h5" -not -name "${nameroot}_v*[0-9]${coreneuron_suffix}_${BUILD_COMPILER}_${BUILD_COMPILER_VERSION}*.h5" 2> /dev/null | sort -V | tail -n 1)
